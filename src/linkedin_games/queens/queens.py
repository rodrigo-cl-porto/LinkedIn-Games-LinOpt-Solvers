...
class Queens(GameBoard):
    ...
    def regions(self, values: set[Region]) -> None:
        ...
        overlapping_squares = {
            square
            for square in all_region_squares
        ...
        nx.set_node_attributes( # Adding a color for each square on the board
            ...
            name="color",
            values={
                (i - 1, j - 1): region.color
                for region in values
                for (i, j) in region.squares
            },
        )
    ...
    def _construct_model(self):
        ...
        I = model.I  # Rows
        J = model.J  # Columns
        K = model.K = pyo.Set(  # Colored Regions
            initialize=[region.color for region in self.regions]
        )

        # COMPOSITE SETS
        S = model.S  # Board Squares
        R = model.R = pyo.Set(  # Region Squares
            K,
            initialize={region.color: region.squares for region in self.regions},
            dimen=2,
        )
        D = model.D = pyo.Set( # Diagonals
    ...
    def _set_solution(self, verbose: bool = False) -> None:
        ...
        crowns = [
            square
            for square, value in nx.get_node_attributes(self.board, "value").items()
            if value == 1
        ]
